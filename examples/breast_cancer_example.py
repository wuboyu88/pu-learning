"""
The goal of this test is to verify that the puLearning really allows a regular estimator to
...
assign them the benign label and consider the benign examples as being \"unlabeled\". We then compare
the performance of the estimator while using the puLearning and without using the puLearning. To
asses the performance, we use the F1 score, precision and recall.

Results show that puLearning greatly increases the performance of an estimator in the case where
...
if __name__ == '__main__':
    ...
    print(len(np.where(y == -1.)[0]), " are benign")
    ...
    pu_f1_scores = []
    pu_weighted_f1_scores = []
    reg_f1_scores = []
    n_sacrifice_iter = range(0, len(np.where(y_train == +1.)[0]) - 46, 5)
    for n_sacrifice in n_sacrifice_iter:
        ...
        # Get f1 score with pu_learning(weighted)
        print("PU learning weighted in progress...")
        estimator = RandomForestClassifier(n_estimators=100,
                                           criterion='gini',
                                           bootstrap=True,
                                           n_jobs=1)
        pu_estimator = WeightedElkanotoPuClassifier(estimator, labeled=sum(y_train_pu == 1),
                                                    unlabeled=sum(y_train_pu == -1))
        pu_estimator.fit(X_train, y_train_pu)
        y_pred = pu_estimator.predict(X_test)
        precision, recall, f1_score, _ = precision_recall_fscore_support(y_test, y_pred)
        pu_weighted_f1_scores.append(f1_score[1])
        print("F1 score: ", f1_score[1])
        print("Precision: ", precision[1])
        print("Recall: ", recall[1])
        print()
    ...
    plt.plot(n_sacrifice_iter, pu_weighted_f1_scores, label='PU Adapted Weighted Random Forest')
